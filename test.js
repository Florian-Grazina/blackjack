...
var btnBet = document.getElementById("start");
...
// var deck = card.concat(card).concat(card).concat(card);
var deck = card;
let i = 9;
...
function drawCardsDealer(a) {
    for (let x = 0; x < a; x++ ){
        ...
        // let i = Math.floor(Math.random()*deck.lenght);
        // deck = deck.splice(index, i)
        totalDealer += cardValue[i];
    }
}
...
function drawCardsPlayer(a) {
    for (let x = 0; x < a; x++ ){
        let i = Math.floor(Math.random()*13);
        totalPlayer += cardValue[i];
        // totalPlayer = 21;
        console.log("player", cardName[i]);
